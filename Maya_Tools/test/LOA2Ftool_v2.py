...
class MyWindow(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Facial Animation Importer LO")
        ...
        button_browseFacalAnim = QtWidgets.QPushButton("Browse")
    ...
    def browseFacialAnimation(self):
        ...
        setting_path = 'S:/Public/qiu_yi/JCQ_Tool/data/LOarkit52_newrig.json'
        ...
        for frame_index in range(numFrames):  # 每一帧
            controller_values = {}
                ...
                if facsName in setting_data:
                    ...
                    for controller, attrs in facs_values.items():
                        for attr, value in attrs.items():
                            controller_values.setdefault(controller, {})

                            if attr in ["scaleX", "scaleY", "scaleZ"]:
                                prev = controller_values[controller].get(attr, 1)
                                controller_values[controller][attr] = prev + weight * (value - 1)
                            else:
                                prev = controller_values[controller].get(attr, 0)
                                controller_values[controller][attr] = prev + weight * value
            ...
            for controller, attrs in controller_values.items():
                for attr, value in attrs.items():
                    ...
                    try:
                        ...
                        affected_controllers.add(controller_with_namespace.split('.')[0])
                    except Exception as e:
