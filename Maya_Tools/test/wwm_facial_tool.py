...
class MyWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        tab1_comboBox_Label = QtWidgets.QLabel("namespace:")
        self.tab1_comboBox_namespace = QComboBox()
        self.namespaceadd(self.tab1_comboBox_namespace)
        self.tab1_comboBox_namespace.setCurrentIndex(0)  # 设置默认选中的项
        self.tab1_comboBox_namespace.setEditable(True)  # 让下拉菜单可编辑
        ...
        Export_Tab.layout.addWidget(tab1_comboBox_Label, 2, 0)
        Export_Tab.layout.addWidget(self.tab1_comboBox_namespace, 2, 1)
        Export_Tab.layout.addWidget(label_costom, 3, 0)
        Export_Tab.layout.addWidget(self.tab1_text_input, 3, 1,1,2)
        Export_Tab.layout.addWidget(button_export, 3, 4)
    ...
    def namespaceadd(self,target):
        ...
        for item in namespace_items:
            target.addItem(item)
        """
        directory_path = "L:\\Projects\\h72\\cutscene\\assets\\chr"
        # 获取指定路径下的所有文件夹名称作为namespace项
        try:
            namespace_items = [name for name in os.listdir(directory_path)
                               if os.path.isdir(os.path.join(directory_path, name))]
        except:
            QtWidgets.QMessageBox.warning(self, "", r"cant access L:\Projects\h72\cutscene\assets\chr,use default ", QtWidgets.QMessageBox.Ok)
            namespace_items =  [
            "wanjia_male:",
            "yidao:",
            "qh_gongjianbing:",
        ...
        # 对比namespace_items和namespaces里面的元素
        for item in namespace_items:
            for ns in namespaces:
                if ns.startswith(item) and ns[len(item):].isdigit():
                    # 如果namespaces中的元素以namespace_items中的任一元素为前缀，并且后面跟随的是数字
                    exnamespace.append(ns)

        # 合并lists并去重
        namespace_items += list(set(exnamespace))
        namespace_items = [item + ":" for item in namespace_items]
        # 添加空字符串作为最后一个元素
        ...
        # 将结果添加到QComboBox中
        target.addItems(namespace_items)
    ...
    def exportfacial(self):
        namespace=self.tab1_comboBox_namespace.currentText()
        ...
        json_file = {}
        if namespace:
            objs = ["{}:{}".format(namespace, obj) for obj in objs]
        for obj in objs:
            ...
            if attributes:
                for attr in attributes:
                    ...
                    if cmds.keyframe(item, query=True, keyframeCount=True) > 0 and attr != "visibility":
                        # 一次性获取所有关键帧的时间和值
                        key_frames = cmds.keyframe(item, query=True, time=(startFrames, endFrames))
                        values = cmds.keyframe(item, query=True, time=(startFrames, endFrames), valueChange=True)

                        if key_frames:
                            data = {"frame": key_frames, "value": values}
            ...
            obj_name_without_namespace = obj.split(':')[-1]
            if attr_data:
                json_file[obj_name_without_namespace] = attr_data

            # save json

        jsonfile = json.dumps(json_file, indent=4, separators=(',', ':'))

        with open(savepath, 'w') as filenew:
