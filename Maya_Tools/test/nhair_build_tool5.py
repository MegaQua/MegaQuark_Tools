...
def create_ik_spline_auto_curve(joint_root):
    ...
    # 不在这里设置 advanced twist / upVector，延后到控制器创建后
    return ikh, auto_curve


def _setup_ikh_upvector_from_xformY(ikh, xform):
    """将 advanced twist 的 upVector 指向 xform 的世界空间 Y 方向。"""
    if not (ikh and xform and cmds.objExists(ikh) and cmds.objExists(xform)): return
    # 开启 Advanced Twist；按你的要求：Vector=5
    cmds.setAttr(ikh + ".dTwistControlEnable", 1)
    cmds.setAttr(ikh + ".dWorldUpType", 5)

    # VectorProduct: (0,1,0) * worldMatrix(xform) -> dWorldUpVector
    vp = cmds.createNode("vectorProduct", n=uniq(ikh + "_upVec_vp"))
    cmds.setAttr(vp + ".operation", 3)          # Matrix * Vector
    cmds.setAttr(vp + ".normalizeOutput", 1)
    cmds.setAttr(vp + ".input1", 0, 1, 0, type="double3")
    cmds.connectAttr(xform + ".worldMatrix[0]", vp + ".matrix", f=True)
    for ax in "XYZ":
        cmds.connectAttr(vp + ".output" + ax, ikh + ".dWorldUpVector" + ax, f=True)
...
def run(prefix=None):
    ...
    try:
        ...
        first_ctrls_in_order = []  # 记录每条曲线的第一个控制器（*_start_con_curve_0）
        for bkp_curve, orig_curve, base_name in zip(backup_curves, orig_curves, base_names):
            ...
            if ctrls and len(ctrls) > 0:
                first_ctrls_in_order.append(ctrls[0])
            else:
                first_ctrls_in_order.append(None)
        ...
        # 5) 现在控制器已存在：把每条 IK 的 upVector 指向对应第一个控制器的 Y 方向
        # 对齐索引：ikhs 与 backup_curves 是一一对应的
        for ikh, ctrl0 in zip(ikhs, first_ctrls_in_order):
            if ikh and ctrl0:
                _setup_ikh_upvector_from_xformY(ikh, ctrl0)
        ...
        print(u"[OK] 完成 | upVector 指向首个控制器Y | 源链:{} | 复制链:{} | sim: sim_GP"
